...
const AI_KB = [
  ...
  { keys:['store','shop','buy merch','buy something','etsy'],
    ans: 'The DFN Designs storefront (Etsy) goes live alongside the DFN Worldwide Hoodie on September 23, 2026. Right now the only live product is the free Reset System book — everything else is pre-launch.',
    nav: { label: 'Join the Waitlist', href: '#join' } },
...
function initContactForm() {
  ...
  form.addEventListener('submit', async e => {
    ...
    // 2. n8n webhook — safe to spread now that "type" isn't already in data.
    try {
      await fetch(DFN.n8nWebhook, {
        method:  'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    if (nav && nav.href) {
      ...
      if (nav.external) { a.target = '_blank'; a.rel = 'noopener'; }
        ...
        a.addEventListener('click', e => {
          ...
          if (el) { e.preventDefault(); close(); window.scrollTo({ top: el.getBoundingClientRect().top + window.scrollY - 80, behavior: 'smooth' }); }
        });
      }
      wrap.appendChild(a);
    }
