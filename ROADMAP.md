...
- ~~DFN AI only exists on the homepage~~ — **done.** The widget is now on
  all 15 pages, and a dedicated `pages/ai.html` explains what it does and,
  honestly, what it isn't yet (see `AI_SYSTEM.md`). Shipping this surfaced
  a real bug worth recording: the chat's navigation buttons had only ever
  been tested from `index.html`, and several used page-relative hrefs
  (`pages/records.html`) that would have silently pointed to the wrong
  place once the same code ran from inside `/pages/` itself. Fixed by
  routing every nav button through the same depth-aware URL resolver
  already built for site search, rather than shipping the widget
  everywhere and finding out from a broken click.
...
8. **A real conversational AI**, per `AI_SYSTEM.md`: a serverless function
   ...
   item that isn't.
9. **A full e-commerce checkout** (cart, inventory, order history) to
